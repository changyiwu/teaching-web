...
- **拆重點時把既有 canvas 引擎參數化，不要複製一份**：一個重點拆成兩個時，互動往往只是「同一份繪圖程式碼、開放不同的模式子集」。把 `initXxxCanvas()` 改成吃 `opts`（`canvasId`、元素 id 前綴、預設模式），呼叫兩次即可。**按鈕的 `querySelectorAll` 一定要收斂到自己那一組**（加 `data-*-group="gcd"` 之類的屬性再選），否則兩張畫布會搶同一批控制項，點 A 組會連 B 組一起改。另外，若某個模式用不到某條滑桿（例如只剩乘／除模式時的「外層指數」），**要把那條滑桿從該張卡片的 HTML 拿掉**並在 JS 加 `if (nS)` 防護，不要留一條永遠停用的死 UI。實測 `1-2-2`、`1-2-4`、`1-3-1` 三頁都適用
...
24. **互動卡數值列（`.value-display`）的長算式要拆成多段 `\( \)`，用 `<wbr>` 接合**：`mjx-container` 是不折行的 inline-block，一整條 `A = B = C` 包成一段時，窄螢幕下整塊都放不下（實測 414px 溢出 42～104px）。作法是**在每個關係／運算子前斷開**，後段以 `{}` 開頭維持該運算子的字距——`\(A\)<wbr>\({}= B\)<wbr>\({}= C\)`；連乘則是 `\(-3\)<wbr>\({}\times (-5)\)…`。接合一定要用零寬的 `<wbr>` 而不是空白：空白會多塞一個字距，`<wbr>` 則讓寬螢幕的排版與原本**逐像素相同**（`1-1-2`／`1-1-3`／`1-1-4`／`1-2-3` 四頁 A/B 實測差 0.00px），只有真的放不下時才斷行。這是開發約束 16 在互動卡裡的具體作法：那裡的欄位比解析卡窄得多（1280px 下只有 356px、414px 下只有 215px），改成顯示型 `\[ \]` 並不適用。共用實作是 `math-canvas.js` 的 `wbrEq()`，斷點是深度 0 的 `=`／`+`／`-`（深度同時算 `{}`、`()`、`[]`，且只斷二元運算子），所以沒有等號的算式也斷得開。**驗收要把每顆模式按鈕都點過、逐行推導的每一步都走過**，不能只量預設狀態——`1-3-1` 的 `11x-2[3x-(5x-4)]` 只在切到那個模式時才出現，前四輪掃描因此都沒抓到（與開發約束 27 的驗收要求是同一件事）
