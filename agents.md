...
- [ ] 階段十：教材頁算式繪圖引擎抽成 `materials/math-canvas.js`（`1-3-1`～`1-3-3` 已接上；`1-2-4` 以前的頁面是舊版分歧的複本，要逐頁比對行為差異才能併）
...
- **把教材頁改吃 `materials/math-canvas.js` 前，要逐符號比對「整個 canvas.js」，不能只比第 2 節**：`1-3-2` 另有一份 `drawEqPanel`／`eqLine` 定義在第 3 節前面，只掃第 2 節會漏掉，接上共用檔後就變成重複宣告。作法是把兩邊的頂層 `function`／`const` 各自抓出來做逐字比對，**確認零差異才刪**，有一個不同就整頁不要動。刪完記得順手清掉描述那些工具的孤兒註解
