...
- [x] 階段十四：第一冊 11 節依〈開發約束〉26 回頭重評重點數（依據為課本 PDF 的主題／例題骨架＋章節架構圖）——7 節維持，4 節重拆：`1-2-2` 6→7（應用問題拆成 GCD 型／LCM 型）、`1-2-4` 6→7（指數律③與④分家）、`1-3-1` 6→7（四則運算與分數形式獨立）、`1-3-3` 7→8（速率與合理性分家）
