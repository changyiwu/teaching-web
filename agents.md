...
- [ ] 階段十二：`.value-display` 在 414px 手機寬度下仍有 5 處 MathJax 算式真溢出（`1-1-2` 重點四 104px、`1-1-3` 重點二 28px 與重點四 8px、`1-1-4` 重點四 95px、`1-2-3` 重點六 42px）。1280px 投影全部正常，只影響手機開；解法依〈開發約束〉20 先想能不能把算式寫短
