...
- 2026-07-23（第二輪）：
  ...
  - `1-1-1`、`1-1-2` 課前漫畫依 `comic-generator` 重生（1080x1350、JSON 對白框）。
  - 教材圖片全面轉 WebP：11.4MB → 0.89MB。
- 2026-07-23（第三輪）：
  - 刪除 `materials/1-1-2` 兩個 `.bak` 本機備份；修正 README 部署說明（Pages 為 `main` 分支自動部署，非未設定）。
  - `1-1-1` 換上依更新後 `comic-generator`（`-TextOnly`／`draw_bubble: false`）重生的漫畫，消除框中框。
  - `1-1-2` 漫畫改回日式動漫風重生，頁面文案與 alt 同步調整。
  - 技能規則兩項調整：移除「裁到精準 4:5」（改為補白邊，避免裁掉四格內容）；漫畫與插圖風格改為依小節指定、同一小節內一致。
